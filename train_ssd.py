...
tf.app.flags.DEFINE_string(
    'data_dir', '/home/weiwei/zoomlion_code/SSD.TensorFlow/dataset/tfrecords',
...
def input_pipeline(dataset_pattern='train-*', is_training=True, batch_size=FLAGS.batch_size):
    def input_fn():
        out_shape = [FLAGS.train_image_size] * 2  # train_image_size: 300
        ...
        anchor_creator = anchor_manipulator.AnchorCreator(out_shape,
                                                          ...
                                                          anchor_ratios=[(1., 2., .5), (1., 2., 3., .5, 0.3333),
                                                                         ...
                                                                         (1., 2., 3., .5, 0.3333),
                                                                         (1., 2., .5), (1., 2., .5)],
                                                          layer_steps=[8, 16, 32, 64, 100,
                                                                       300])  # 在特征图上移动一个像素相当于原图移动像素数目
        ...
        image_preprocessing_fn = lambda image_, labels_, bboxes_: ssd_preprocessing.preprocess_image(
            image_, labels_, bboxes_, out_shape, is_training=is_training,
            data_format=FLAGS.data_format, output_rgb=False)     # data_format=channel_first
        ...
        image, _, shape, loc_targets, cls_targets, match_scores = dataset_common.slim_get_batch(
            FLAGS.num_classes, batch_size, ('train' if is_training else 'val'),
            os.path.join(FLAGS.data_dir, dataset_pattern),
            FLAGS.num_readers, FLAGS.num_preprocessing_threads, image_preprocessing_fn, anchor_encoder_fn,
            num_epochs=FLAGS.train_epochs, is_training=is_training)

        global global_anchor_info  # 使用 global，为定义在函数外的变量赋值
        global_anchor_info = {
...
def modified_smooth_l1(bbox_pred, bbox_targets, bbox_inside_weights=1., bbox_outside_weights=1., sigma=1.):
    ...
    with tf.name_scope('smooth_l1', [bbox_pred, bbox_targets]):
        ...
        # 查看对应元素相减结果绝对值比 1 大还是比 1 小
        # 差值绝对值比 1 小返回 True，否则返回 False
...
def ssd_model_fn(features, labels, mode, params):
    ...
    num_anchors_per_layer = global_anchor_info['num_anchors_per_layer']  # 5776+2166+600+150+40=8732
    ...
    with tf.variable_scope(params['model_scope'], default_name=None, values=[features], reuse=tf.AUTO_REUSE):   # params['model_scope']:'ssd300'
        ...
        location_pred, cls_pred = ssd_net.multibox_head(feature_layers, params['num_classes'], all_num_anchors_depth,
                                                        data_format=params[
                                                            'data_format'])  # (8,16,38,38), (8,84,38,38)  21*4=84,21*6=126

        if params['data_format'] == 'channels_first':
            cls_pred = [tf.transpose(pred, [0, 2, 3, 1]) for pred in cls_pred]
            location_pred = [tf.transpose(pred, [0, 2, 3, 1]) for pred in location_pred]

        cls_pred = [tf.reshape(pred, [tf.shape(features)[0], -1, params['num_classes']]) for pred in
                    cls_pred]  # 行数表示该层特征图预测框数(5776)，列数表示类别数(21)
        ...
        cls_pred = tf.reshape(cls_pred, [-1, params['num_classes']])  # 行数为 batch 数目乘以每张图像的 default box 数目
    ...
    with tf.device('/cpu:0'):
        ...
        with tf.control_dependencies([cls_pred, location_pred]):
            with tf.name_scope('post_forward'):
                ...
                tf.identity(cls_accuracy[1], name='cls_accuracy')  # 创建一个新的 op，加入日志
    ...
    total_loss = tf.add(cross_entropy + loc_loss, tf.multiply(params['weight_decay'], tf.add_n(l2_loss_vars),
                                                              name='l2_loss'), name='total_loss')
    ...
    return tf.estimator.EstimatorSpec(
        ...
        scaffold=tf.train.Scaffold(init_fn=get_init_fn()))  # tf.train.Scaffold 进行模型初始化
...
def main(_):
    ...
    # tf.contrib.estimator.replicate_model_fn 进行多 GPU 并行　
    ...
    # 用字典存储想要打印的张量
    # 每个键值只不过是用于日志输出的一个别名，它的值则是 TensorFlow 计算图中的某个张量的名称。
    tensors_to_log = {
        'lr': 'learning_rate',
    ...
    # tf_debug.TensorBoardDebugHook('weiwei:6064')
